...
class Global_model extends CC_Model {
    ...
    public function count_total_blog_by_user_id($user_id) {
        $result = $this->db->get_where($this->_blogs, array('user_id' => $user_id, 'deletion_status' => 0));
        return $result->num_rows();
    }
    public function count_total_notes_by_user_id($user_id) {
        $result = $this->db->get_where($this->_notes, array('user_id' => $user_id, 'deletion_status' => 0));
        return $result->num_rows();
    }

    public function get_all_categories() {
        $result = $this->db->limit(25)->order_by('sort_id', 'asc')->get_where($this->_cat, array('publication_status' => 1, 'deletion_status' => 0, 'parent_id' => 0));
