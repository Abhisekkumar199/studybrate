...
class Exams extends CC_Controller {
    ...
    public function add_exam() {
        ...
        if (!empty($data['listing_info'])) {
            ...
            $sdata['exception'] = 'Please first add your Profile.';
            $this->session->set_userdata($sdata);
            redirect('user/listing/add_listing', 'refresh');
        }
    }

    public function store_exam_details() {
        $config = array(
            array(
                'field' => 'product_name',
                'label' => 'product name',
      ...
      //  }
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          //  $data['image_path'] = $this->add_picture();
            ...
            if (!empty($insert_id)) {
                $sdata['success'] = 'Exam added successfully. ';
                $this->session->set_userdata($sdata);
                redirect('user/exams', 'refresh');
            } else {
                $sdata['exception'] = 'Something went wrong!! Please try again.';
                $this->session->set_userdata($sdata);
                redirect('user/exams/add_exams', 'refresh');
            }
        }
    }
